...
namespace WalletWasabi.Tests.UnitTests.WabiSabi.Client
{
	public class BobClientTests
	{
		...
		public async Task RegisterOutputTestAsync()
		{
			...
			Task confirmationTask = aliceClient.ConfirmConnectionAsync(TimeSpan.FromSeconds(1), new[] { coin1.EffectiveValue(round.FeeRate).Satoshi }, new[] { round.MaxVsizeAllocationPerAlice - (long)coin1.ScriptPubKey.EstimateInputVsize() }, round.MaxVsizeAllocationPerAlice, CancellationToken.None);
