...
{
	public delegate ScalarVector SecretNonceSequence();
	...
	public sealed class Transcript
	{
		...
		public /*Func<IEnumerable<Scalar>>*/ SecretNonceSequence CreateSyntheticNocesProvider(IEnumerable<Scalar> secrets, WasabiRandom random)
		{
			...
			ScalarVector SecretNoncesGenerator()
			{
				while (true)
				{
					return new ScalarVector(secrets.Select(_ => new Scalar(forked.Prf(KeySizeInBytes, false))));
				}
			};

			// Generate a new scalar for each secret using this updated state as a seed.
			return SecretNoncesGenerator;
		}

		public void CommitStatement(IStatement statement)
		{
			// FIXME add lengths and in general de-uglify
			AddMessages(StatementTag, statement.PublicPoints.Select(x => x.ToBytes()).Concat(statement.Generators.SelectMany(xs => xs.Select(x => x.ToBytes()))));
		}

		public void CommitPublicNonces(IEnumerable<GroupElement> publicNonces)
		{
			CryptoGuard.NotInfinity(nameof(publicNonces), publicNonces);
