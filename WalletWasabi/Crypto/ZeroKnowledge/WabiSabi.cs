...
namespace WalletWasabi.Crypto.ZeroKnowledge
{
	...
	public static class Proofs { // TODO rename
		public static IProof Prove(IFSProver prover, WasabiRandom random) {
			var transcript = new Transcript(); // TODO also accept initialized prover and clone for sanity check?
			...
			if (!Verify(prover.ToVerifier(), proof))
			{
		...
		public static bool Verify(IFSVerifier verifier, IProof proof) {
	...
	public interface IStatement : IVerifier { // optionally ISimulator
		...
		IEnumerable<GroupElements> Generators { get; }
	}
...
namespace WalletWasabi.Crypto.ZeroKnowledge.FSTransform
{
	...
	public class Verifier : IFSVerifier
	{
		...
		private Proof VerifyResponse(Transcript transcript, Proof proof)
		{
			var challenge = transcript.GenerateChallenge();
...
namespace WalletWasabi.Crypto.ZeroKnowledge.Or
{
	...
	public class Verifier : IFSVerifier
	{
		...
		private RespondToChallenge CreateProof(Transcript transcript, CompoundProof proofs)
		{
			var allPublicNonces = proof.Select(x => ((Proof)x).PublicNonces);
			...
			foreach (var (statement, proof_iface, publicNonces) in Statements.Zip(proofs).Zip(rotatedNonces))
			{
				...
				var fork = transcript.MakeCopy();
				fork.CommitPublicNonce(publicNonces);
				var challenge = fork.GenerateChallenge();
			...
			// commit to public nonces to make challenges of any subsequent
			transcript.CommitPublicNonces(allPublicNonces);
...
namespace WalletWasabi.Crypto.ZeroKnowledge.And
{
	...
	public class Verifier : IFSVerifier
	{
		...
		private RespondToChallenge Commit(CompoundProof proofs, IEnumerable<ProverCommitToNonces> commitDelegates)
		{
			...
			// return the Respond delegate
			return delegate() {
				return this.VerifyResponses(proofs, respondDelegates);
			};
		}

		private bool VerifyResponses(CompoundProof proofs, IEnumerable<VerifyResponse> verifyDelegates)
		{
			return verifyDelegates.ZipForceEqualLength(proofs).All((verify, proof) => verify(proof));
		}
	}
}
...
namespace WalletWasabi.Crypto.ZeroKnowledge.LinearRelation
{
	...
	public class Equation
	{
		...
		public GroupElements Generators { get; }
		...
		public GroupElement Simulate(Scalar challenge, IEnumerable<Scalar> fakeResponses)
		{
			...
			//   R = sG - eP
			return challenge * PublicPoint - fakeResponses * GenerGenerators;
		}
	...
	public class Statement : ISimulatableStatement
	{
		...
		{
			return Equations.ZipForceEqualLength(allFakeResponses, (eqn, responses) => eqn.Simulate(challenge, responses));
		}
	}
